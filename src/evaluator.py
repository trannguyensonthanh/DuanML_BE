...
class GoldEvaluator:
    ...
    def run(self):
        ...
        for true_label_str in folder_classes:
            ...
            for img_name in tqdm(image_files, desc=f"Testing {true_label_str}"):
                ...
                for idx, line in enumerate(lines):
                    ...
                    # Lấy xác suất của tất cả các lớp (VD: [0.1, 0.8, 0.05, 0.05])
                    ...
                    # Thêm xác suất từng class vào CSV luôn
                    ...
                    if not is_correct:
                        # Tên file rất quan trọng để debug
                        # VD: Error_True[Metal]_Pred[Plastic]_Conf[85]_img01.jpg
                        err_fname = f"Err_True[{true_label_str}]_Pred[{pred_label_str}]_Conf[{int(confidence*100)}]_{img_name}"
                        # Vẽ thêm text lên ảnh để dễ nhìn
                        debug_img = processed_img.copy()
                        # Resize to bigger for viewing
                        debug_img = cv2.resize(debug_img, (256, 256))
                        cv2.putText(debug_img, f"T:{true_label_str}", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                        cv2.putText(debug_img, f"P:{pred_label_str}", (5, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        ...
        # Top-2 Accuracy (Nếu đáp án đúng nằm trong 2 dự đoán cao nhất)
        # Cần chuyển y_true sang dạng số
        ...
        print("═"*60)
        print(f"🏆 Top-1 Accuracy (Độ chính xác tuyệt đối):  {acc*100:.2f}%")
        ...
        # 3. Vẽ biểu đồ phân tích (Visual Analytics)
    ...
    def _visualize_performance(self, y_true, y_pred, df):
        ...
        # B. Confidence Distribution (Phân phối độ tin cậy)
        # Biểu đồ này cho biết: Khi đoán Đúng và Sai, mô hình tự tin thế nào?
